datos=readtable("20230301_DLIM_Freno_v4.xlsx");

frecuencia_freno=-48:3:-3;
slip_freno=1.1:0.1:2;
...
for i=1:max(size(slip_freno))
    for j=1:max(size(frecuencia_freno))
           Aux_FuerzaX_freno(i,j)=datos{valor,6};
           valor=valor-1;
    end
end
...
for i=1:max(size(slip_freno))
     FuerzaX_freno(i,:)=-Aux_FuerzaX_freno(max(size(slip_freno))+1-i,:);
end
